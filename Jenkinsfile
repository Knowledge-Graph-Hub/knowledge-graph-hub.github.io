    ...
    stages {
        ...
        stage('Make manifest') {
            steps {
                dir('./gitrepo') {
                    script {
                        def run_make_manifest = sh(script: '. venv/bin/activate && cd utils/ && AWS_CONFIG_FILE=./awscli_config.txt python make_kg_manifest.py --bucket kg-hub-public-data --outpath MANIFEST.yaml', returnStatus: true
                            )
